...
Milvus is the most popular open source vector database that focus on vectors store, query and similarity search. provides cloud scalability, high reliability and great performance. This example is modified based on the Faiss demo and shows the usage of Milvus index.
...
3. Create the index with [`Dataset.add_milvus_index`]:

```py
>>> ds_with_embeddings.add_milvus_index(
    host="localhost",
    port="19530",
    index_name="milvus_index",
    column="embeddings",
    dim=768,
    milvus_collection_name="hf_dataset_milvus_index",
)
```

`host` and `port` is the address of Milvus service. You can start Milvus on your machine, or see [Milvus installation guide](https://milvus.io/docs/install_standalone-operator.md) if you don’t already have it installed..

`add_milvus_index` will create a collection and a index in Milvus, and insert data into it. Compared with Faiss, Milvus can not only store vector data but also various scalar data. By default, `add_milvus_index` will create a collection based on dataset's schema and a `IVF_FLAT` index. You can specific the collection schema and index type by pass optional parameters. Please see [Create collection](https://milvus.io/docs/create_collection.md) and [Build index](https://milvus.io/docs/build_index.md) for more detail.
...
>>> question = "Is it serious ?"
>>> question_embedding = q_encoder(**q_tokenizer(question, return_tensors="pt"))[0][0].numpy()
>>> scores, retrieved_examples = ds_with_embeddings.get_nearest_examples('milvus_index', question_embedding, k=10)
>>> retrieved_examples["line"][0]
'_that_ serious? It is not serious at all. It’s simply a fantasy to amuse\r\n'
```

5. You can access the index with [`Dataset.get_index`] and use it for special operations, e.g. query by id:

```py
>>> milvus_index = ds_with_embeddings.get_index('milvus_index')
>>> query_result = milvus_index.query(query(expr="pk in [1,3,5,7,9]", limit=3))
```

6. Data will persist on Milvus whenever you create a Milvus index. If you want to delete the milvus data physically, use [`MilvusIndex.remove`]:

```py
>>> ds_with_embeddings.get_index('milvus_index').remove()
>>> ds_with_embeddings.drop_index("milvus_index")
```

7. Reload it at a later time with [`Dataset.load_milvus_index`]:

```pygi
>>> ds.load_milvus_index(index_name='milvus_index_new',  milvus_collection_name="hf_dataset_milvus_index")
```
