...
class MilvusIndexException(Exception):
    pass
...
class MilvusIndex(BaseIndex):
    ...
    def __init__(
        ...
        milvus_collection_name = milvus_collection_name or "hf_datasets_" + os.path.basename(tempfile.NamedTemporaryFile().name)

        if utility.has_collection(milvus_collection_name):
            self.__get_collection__(milvus_collection_name, milvus_collection_schema, milvus_index_params)
        else:
            self.__create_collection__(milvus_collection_name, milvus_collection_schema, milvus_index_params)

    def __get_collection__(
            self,
            milvus_collection_name: Optional[str] = None,
            milvus_collection_schema: Optional[list] = None,
            milvus_index_params: Optional[dict] = None,
        ...
        # get exist collection
        milvus_collection = Collection(milvus_collection_name)
        self.collection = milvus_collection

        # check schema
        if milvus_collection_schema is not None:
            if milvus_collection.schema != milvus_collection_schema:
                raise MilvusIndexException(f"milvus collection {milvus_collection_name} already exist"
                                           f" and have a different schema")

        # get schema
        self.schema = milvus_collection.schema

        # check index
        if milvus_collection.has_index():
            for index in milvus_collection.indexes:
                if milvus_index_params is not None:
                    if index.params == milvus_index_params:
                        self.milvus_index_params = index.params
                        self.milvus_vector_field = self.milvus_index_params["field"]
                    else:
                        raise MilvusIndexException(f"milvus collection {milvus_collection_name} already has index"
                                                   f" and has different definition {index.params}")
                else:
                    self.milvus_index_params = index.params
                    self.milvus_vector_field = self.milvus_index_params["field"]
        else:
            self.milvus_index_params = milvus_index_params or {
                "index_type": "IVF_FLAT",
                "metric_type": "L2",
                "params": {"nlist": 128},
            }
            if "field" in self.milvus_index_params:
                self.milvus_vector_field = self.milvus_index_params["field"]
    ...
    def _parse_field(self, params: dict):
        ...
        if dtype == "STRING":
            ...
            params['max_length'] = params.pop("max_length", 65535)
        if dtype == "VARCHAR":
            dtype = DataType.VARCHAR
    ...
    def insert(self, entities: Union[np.array, "Dataset"], external_arrays: Union[np.array, "Dataset"]=None,
               batch_size: int = 1000, use_dataset_index_as_pk: bool = True):
        if entities is None:
            return

        if use_dataset_index_as_pk:
            indice = [i + self.collection_size for i in range(len(entities))]

        if external_arrays is not None and len(external_arrays) != len(entities):
            raise Exception(message=f"The length of entities and external_arrays are not match, "
                                    f"entities: {len(entities)}, external_arrays {len(entities)}")
        ...
        while inserted_size < need_insert_size:
            ...
            if use_dataset_index_as_pk:
                pk = indice[inserted_size: inserted_size + this_batch_size]
                batch.append(pk)
            for i in range(0, len(self.column_names())):
                if self.column_names()[i] == "pk":
                    continue
                column = []

                if self.column_names()[i] not in entities.column_names and external_arrays is not None:
                    for j in range(inserted_size, inserted_size + this_batch_size):
                        if isinstance(external_arrays, datasets.Dataset):
                            column.append(external_arrays[j][self.column_names()[i]])
                        elif self.column_names()[i] == self.milvus_vector_field:
                            column.append(external_arrays[j])
                        else:
                            column.append(external_arrays[j][i])
                else:
                    for j in range(inserted_size, inserted_size + this_batch_size):
                        if isinstance(entities, datasets.Dataset):
                            column.append(entities[j][self.column_names()[i]])
                        elif self.column_names()[i] == self.milvus_vector_field:
                            column.append(entities[j])
                        else:
                            column.append(entities[j][i])
                batch.append(column)
    ...
    def query(self, expr: str, partition_names=None, **kwargs) -> SearchResults:
        """Query the dataset based on filters.
        ...
        Args:
            ...
            partition_names: (``List[str]``, optional):
                A list of partition names to query in. Defaults to None.
            **kwargs (``dict``, optional):
                ...
                * *limit* (``int``)
                    Combined with limit to enable pagination
            See https://milvus.io/docs/query.md for detail

        Output:
            List, contains all results
        """
        self.collection.load()
        query_result = self.collection.query(expr, self.column_names(), partition_names, **kwargs)
        return query_result
...
class IndexableMixin:
    ...
    def add_milvus_index(
        ...
        milvus_index_params: Optional[dict] = None,
        batch_size: int = 1000,
        use_dataset_index_as_pk: bool = True,
    ):
        ...
        Args:
            ...
            host (Optional `str`, defaults to localhost):
                host of where Milvus is running
            port (Optional `int`, defaults to 19530):
                port of where Milvus is running
            ...
            use_dataset_index_as_pk ('bool'): whether generate auto_id
        """
        ...
        milvus_index.insert(entities=self, batch_size=batch_size, use_dataset_index_as_pk=use_dataset_index_as_pk)
    ...
    def add_milvus_index_from_external_arrays(
        ...
        milvus_index_params: Optional[dict] = None,
        batch_size: int = 1000,
        use_dataset_index_as_pk: bool = False,
    ):
        """Add an index using Milvus for fast retrieval.
        ...
        Args:
            ...
            milvus_index_params (Optional `dict`):
                ...
                Milvus support various index type to trade-off performance and cost.
                Please see https://milvus.io/docs/build_index.md for more detail
            batch_size (`int`): Size of the batch to use while adding vectors to the Milvus. Default is 1000.
            use_dataset_index_as_pk ('bool'): whether generate auto_id
        """
        ...
        if external_arrays is not None:
            milvus_index.insert(self, external_arrays.astype(dtype),
                                batch_size=batch_size, use_dataset_index_as_pk=use_dataset_index_as_pk)
        self._indexes[index_name] = milvus_index
    ...
    def load_milvus_index(
        ...
        """Load an existing index using Milvus for fast retrieval.

        Args:
            index_name (`str`):
                The `index_name`/identifier of the index. This is the index name that is used to call `get_nearest` or `search`.
            milvus_collection_name (`str`):
                The name of Milvus collection to load.
            host (`str`, *optional*, defaults to `localhost`):
                Host of where Milvus is running.
            port (`str`, *optional*, defaults to `19530`):
                Port of where Milvus is running.
            milvus_collection_schema (Optional `List`):
                The schema of the Milvus collection.
            milvus_index_params (Optional `dict`):
                The configuration of the Milvus index.
        """
        self._indexes[index_name] = MilvusIndex(
            host=host, port=port, milvus_collection_name=milvus_collection_name,
            milvus_collection_schema=milvus_collection_schema, milvus_index_params=milvus_index_params)
