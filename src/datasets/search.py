...
class MilvusIndex(BaseIndex):
    ...
    You can find more information about Milvus here:
    https://milvus.io/
    """
    ...
    def __init__(
        ...
        if not _has_milvus:
            raise ImportError(
                "You must install Milvus to use MilvusIndex. To do so you can run `pip install pymilvus==2.2.0 for example`"
            )
    ...
    def insert(self, entities: Union[np.array, "Dataset"], batch_size: int = 1000):
        ...
        insert_size = 0
        need_insert_size = len(entities[0])
        while insert_size < need_insert_size:
            if insert_size + batch_size <= need_insert_size:
                batch = []
                for i in range(0, len(entities)):
                    batch.append(entities[i][insert_size:(insert_size + batch_size)])
            else:
                batch = []
                for i in range(0, len(entities)):
                    batch.append(entities[i][insert_size:need_insert_size])
            insertResult = self.collection.insert(batch)
            insert_size = insert_size + len(batch[0])
            self.collection_size = self.collection_size + len(batch[0])
    ...
    def search(self, query: np.array, k=10) -> SearchResults:
        ...
        search_result = self.collection.search(query, self.milvus_vector_field, self.milvus_index_params, k)
    ...
    def search_batch(self, queries: np.array, k=10) -> BatchedSearchResults:
        ...
        self.collection.load()
        search_result = self.collection.search(queries, self.milvus_vector_field, self.milvus_index_params, k)
        distances = []
        ids = []
